...
Workflow actions carry typed fulfillment or delivery choices, an explicit basket confirmation, or a provider intent/request reference. They do not carry MCP command names or arbitrary tool arguments. Approval and decline actions always target an explicit `approvalId`.
