...
`connected_provider_accounts` stores provider sync metadata: first sync, last sync, stale timestamp, current sync status and the last redacted error. `provider_sync_events` is household-scoped and retains per-attempt request/response summaries and error state. `memory_initializations` is user-scoped so it can exist before household onboarding and links the user, optional member, optional provider account and source outbox event.
