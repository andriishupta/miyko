...
export const toContractPlanningRun = (row: { id: string; householdId: string; startedByMemberId: string; langgraphThreadId: string | null; langgraphRunId: string | null; status: PlanningRun['status']; startedAt: Date | null; pausedAt: Date | null; completedAt: Date | null }): PlanningRun => ({ id: row.id, householdId: row.householdId, startedByMemberId: row.startedByMemberId, langgraphThreadId: row.langgraphThreadId, langgraphRunId: row.langgraphRunId, status: row.status, startedAt: iso(row.startedAt), pausedAt: iso(row.pausedAt), completedAt: iso(row.completedAt) })
