...
const planSchema = z.object({ title: z.string().min(1).max(120), meals: z.array(z.object({ name: z.string().min(1).max(160), ingredients: z.array(z.string().min(1).max(160)).max(30) }).strict()).max(14) }).strict()
...
const memoryGraph = new StateGraph(MemoryState)
  .addNode('load_receipts', async (state) => ({ normalizedReceipts: JSON.stringify(state.receipts.slice(0, 100)) }))
  .addNode('analyze_preferences', async (state) => ({ analysis: await openAiClient.json([
  ...
  .addNode('write_mem0', async (state) => ({ memoryId: state.analysis ? (state.memoryId ? (await mem0Client.update(state.namespace, state.memoryId, state.analysis.summary), state.memoryId) : await mem0Client.add(state.namespace, state.analysis.summary, { source: 'receipt_initialization', scope: 'member' })) : null }))
  .addEdge(START, 'load_receipts')
  .addEdge('load_receipts', 'analyze_preferences')
  .addEdge('analyze_preferences', 'write_mem0')
  .addEdge('write_mem0', END)
...
const createFoodGraph = (searchProducts: (query: string) => Promise<unknown[]>) => new StateGraph(FoodState)
  .addNode('classify_request', async (state) => ({ classification: await openAiClient.json([
    { role: 'system', content: 'Classify a household food request. Return only JSON with type, summary and confidence.' },
    { role: 'user', content: state.text },
  ], classificationSchema) }))
  .addNode('load_memory', async (state) => ({ memory: await mem0Client.search(state.namespace, state.text, 20) }))
  .addNode('load_provider_history', async (state) => ({ providerHistory: state.providerHistory.slice(0, 100) }))
  .addNode('plan_meals', async (state) => ({ plan: await openAiClient.json([
  ...
  .addNode('search_products', async (state) => ({ products: state.plan ? (await Promise.all(state.plan.meals.flatMap((meal) => meal.ingredients).map((ingredient) => searchProducts(ingredient)))).flat() : [] }))
...
export const agentLayer: AgentLayer = {
  async initializeMemory(input: { namespace: string; receipts: unknown[]; memoryId?: string | null }) {
  ...
  async foodIntent(input: { text: string; namespace: string; providerHistory: unknown[]; searchProducts: (query: string) => Promise<unknown[]> }) {
    const threadId = `food:${randomUUID()}`
    const { searchProducts, ...state } = input
    const result = await createFoodGraph(searchProducts).invoke(state, { configurable: { thread_id: threadId } })
