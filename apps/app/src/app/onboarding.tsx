  ...
  return (
    <ScreenScroll bottomInset={insets.bottom + Spacing.four} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingTop: insets.top + Spacing.four }}>
      <View style={{ gap: Spacing.five }}>
        ...
        {step === 2 && <Surface><MiykoText variant="section">Connect a store provider</MiykoText><MiykoText variant="body" color="textSecondary">Connect Silpo to start provider-dependent workflows. Provider credentials are sent to the API and are not stored in the app.</MiykoText><ProviderManagement allowBind onComplete={() => router.replace('/home')} /><SecondaryButton label="Skip for now" onPress={() => router.replace('/home')} /></Surface>}
