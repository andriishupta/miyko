  ...
  return (
    <ScreenScroll bottomInset={insets.bottom + Spacing.four} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingTop: insets.top + Spacing.four }}>
      <View style={{ gap: Spacing.five }}>
        <MiykoLogo />
        <View style={{ gap: Spacing.two }}><MiykoText variant="caption" color="accent">STEP {step} OF 2</MiykoText><MiykoText variant="hero">{step === 1 ? 'Set up your household.' : 'Connect your store account.'}</MiykoText><MiykoText variant="body" color="textSecondary">{step === 1 ? 'Your account is ready. Choose how you want to get started with MiyKo.' : 'Connect your store account to make shopping plans together.'}</MiykoText></View>
        {step === 1 && <>
          ...
          {choice === 'create' && <Surface><MiykoText variant="section">Create household</MiykoText><MiykoText variant="body" color="textSecondary">Create the shared space for food plans and decisions.</MiykoText><Field label="HOUSEHOLD NAME" placeholder="e.g. Petrenko family" value={householdName} onChangeText={setHouseholdName} />{error && <MiykoText variant="caption" color="danger">{error}</MiykoText>}<PrimaryButton label="Create household" loading={loading} onPress={() => void createHousehold()} /></Surface>}
        ...
        {step === 2 && <Surface><MiykoText variant="section">Connect your store account</MiykoText><MiykoText variant="body" color="textSecondary">Connect Silpo to start making shopping plans. Your store password stays in the secure store sign-in.</MiykoText><ProviderManagement onComplete={() => router.replace('/home')} /></Surface>}
