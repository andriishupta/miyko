    ...
    <ScreenScroll bottomInset={insets.bottom + Spacing.four} contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', paddingTop: insets.top + Spacing.four }}>
      <View style={{ gap: Spacing.five }}>
        ...
        {step === 1 && <>
          ...
          {choice === 'create' && <Surface><MiykoText variant="section">Create household</MiykoText><MiykoText variant="body" color="textSecondary">Create the shared space for food workflows and approvals.</MiykoText><Field label="HOUSEHOLD NAME" placeholder="e.g. Petrenko family" value={householdName} onChangeText={setHouseholdName} />{error && <MiykoText variant="caption" color="danger">{error}</MiykoText>}<PrimaryButton label="Create household" loading={loading} onPress={() => void createHousehold()} /></Surface>}
        ...
        {step === 2 && <Surface><MiykoText variant="section">Connect a store provider</MiykoText><MiykoText variant="body" color="textSecondary">Connect Silpo to start food workflows. Provider credentials are sent to the API and are not stored in the app.</MiykoText><ProviderManagement allowBind onComplete={() => router.replace('/home')} /></Surface>}
